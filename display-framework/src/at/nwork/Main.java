...
public class Main {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        UdpScreen screen = new UdpScreen(28, 5, "192.168.11.62", 1337);
        int offset = screen.getWidth();
        BasicFont[] text = BasicFont.createText("1234567890");
        int totalLength = 0;
        ...
        while (true) {
            ...
            for (BasicFont c : text) {
                ...
                for (Pixel p : c.getPixels()) {
                    if (p.getX() + hoffset >= 0 && p.getX() + hoffset < screen.getWidth()
                            && p.getY() + voffset < screen.getHeight()) {
